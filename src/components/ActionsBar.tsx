              ...
              <>
                ...
                <div className="absolute right-0 top-full mt-1 z-20 bg-card border border-border rounded-lg shadow-lg py-1 min-w-32">
                  {(['newest', 'oldest', 'az', 'za'] as FileSortMode[]).map(mode => (
                    <button
                      key={mode}
                      onClick={() => { setFileSort(mode); setPage(0); setSortOpen(false); }}
...
function ActionWidget({ action, files, onRun, onShowCode, onDelete, onDownload, devMode, running, disabled }: {
  action: Action;
...
export default function ActionsBar() {
  const { actions, runAction, showActionCode, deleteAction, devMode, runningActionId, files, filesByAction, downloadFile } = useActions();
