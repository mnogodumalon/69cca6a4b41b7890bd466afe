    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Rolle / Abteilung</Label>
            <p className="text-sm">{record.fields.rolle ?? '—'}</p>
          </div>
          <div className="space-y-1">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm whitespace-pre-wrap">{record.fields.notizen_mitarbeiter ?? '—'}</p>
          </div>
        </div>
      </DialogContent>
    </Dialog>
