...
export function RechnungsverwaltungDialog({ open, onClose, onSubmit, defaultValues, kundenverwaltungList, mitarbeiterverwaltungList, leistungskatalogList, enablePhotoScan = true, enablePhotoLocation = true }: RechnungsverwaltungDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'rechnungsverwaltung');
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="leistungen_ref" entity="Leistungskatalog">\n${JSON.stringify(leistungskatalogList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "rechnungsnummer": string | null, // Rechnungsnummer\n  "rechnungsdatum": string | null, // YYYY-MM-DD\n  "faelligkeitsdatum": string | null, // YYYY-MM-DD\n  "rechnungsstatus": LookupValue | null, // Rechnungsstatus (select one key: "entwurf" | "versendet" | "bezahlt" | "ueberfaellig" | "storniert") mapping: entwurf=Entwurf, versendet=Versendet, bezahlt=Bezahlt, ueberfaellig=Überfällig, storniert=Storniert\n  "kunde_ref": string | null, // Display name from Kundenverwaltung (see <available-records>)\n  "mitarbeiter_ref": string | null, // Display name from Mitarbeiterverwaltung (see <available-records>)\n  "leistungen_ref": string | null, // Display name from Leistungskatalog (see <available-records>)\n  "menge": number | null, // Menge / Stunden\n  "nettobetrag": number | null, // Nettobetrag (€)\n  "steuersatz_rechnung": LookupValue | null, // Steuersatz (select one key: "steuersatz_19" | "steuersatz_7" | "steuersatz_0") mapping: steuersatz_19=19%, steuersatz_7=7%, steuersatz_0=0%\n  "bruttobetrag": number | null, // Bruttobetrag (€)\n  "zahlungsdatum": string | null, // YYYY-MM-DD\n  "zahlungsart": LookupValue | null, // Zahlungsart (select one key: "ueberweisung" | "lastschrift" | "kreditkarte" | "bar" | "paypal") mapping: ueberweisung=Überweisung, lastschrift=Lastschrift, kreditkarte=Kreditkarte, bar=Bar, paypal=PayPal\n  "notizen_rechnung": string | null, // Notizen\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["kunde_ref", "mitarbeiter_ref", "leistungen_ref"]);
        ...
        if (kunde_refName) {
          const kunde_refMatch = kundenverwaltungList.find(r => matchName(kunde_refName!, [String(r.fields.kundennummer ?? '')]));
          if (kunde_refMatch) merged['kunde_ref'] = createRecordUrl(APP_IDS.KUNDENVERWALTUNG, kunde_refMatch.record_id);
        }
        const mitarbeiter_refName = raw['mitarbeiter_ref'] as string | null;
        if (mitarbeiter_refName) {
          const mitarbeiter_refMatch = mitarbeiterverwaltungList.find(r => matchName(mitarbeiter_refName!, [String(r.fields.mitarbeiter_vorname ?? '')]));
        ...
        const leistungen_refName = raw['leistungen_ref'] as string | null;
        if (leistungen_refName) {
          const leistungen_refMatch = leistungskatalogList.find(r => matchName(leistungen_refName!, [String(r.fields.leistungsbezeichnung ?? '')]));
          if (leistungen_refMatch) merged['leistungen_ref'] = createRecordUrl(APP_IDS.LEISTUNGSKATALOG, leistungen_refMatch.record_id);
        }
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="rechnungsstatus">Rechnungsstatus</Label>
            <Select
              value={lookupKey(fields.rechnungsstatus) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, rechnungsstatus: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="rechnungsstatus"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={v => setFields(f => ({ ...f, kunde_ref: v === 'none' ? undefined : createRecordUrl(APP_IDS.KUNDENVERWALTUNG, v) }))}
            >
              ...
              <SelectContent>
                ...
                {kundenverwaltungList.map(r => (
                  <SelectItem key={r.record_id} value={r.record_id}>
                    {r.fields.kundennummer ?? r.record_id}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="space-y-2">
            <Label htmlFor="mitarbeiter_ref">Zuständiger Mitarbeitender</Label>
            <Select
          ...
          <div className="space-y-2">
            <Label htmlFor="leistungen_ref">Leistung</Label>
            <Select
              value={extractRecordId(fields.leistungen_ref) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, leistungen_ref: v === 'none' ? undefined : createRecordUrl(APP_IDS.LEISTUNGSKATALOG, v) }))}
            >
              <SelectTrigger id="leistungen_ref"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
          ...
          <div className="space-y-2">
            <Label htmlFor="menge">Menge / Stunden</Label>
            <Input
              id="menge"
              type="number"
              value={fields.menge ?? ''}
          ...
          <div className="space-y-2">
            <Label htmlFor="steuersatz_rechnung">Steuersatz</Label>
            <Select
              value={lookupKey(fields.steuersatz_rechnung) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, steuersatz_rechnung: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="steuersatz_rechnung"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                <SelectItem value="steuersatz_19">19%</SelectItem>
                <SelectItem value="steuersatz_7">7%</SelectItem>
                <SelectItem value="steuersatz_0">0%</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            <Label htmlFor="zahlungsdatum">Zahlungseingangsdatum</Label>
            <Input
              id="zahlungsdatum"
              type="date"
          ...
          <div className="space-y-2">
            <Label htmlFor="notizen_rechnung">Notizen</Label>
            <Textarea
              id="notizen_rechnung"
              value={fields.notizen_rechnung ?? ''}
              onChange={e => setFields(f => ({ ...f, notizen_rechnung: e.target.value }))}
              rows={3}
            />
          </div>
          <DialogFooter>
