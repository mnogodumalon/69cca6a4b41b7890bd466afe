...
export function LeistungskatalogDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: LeistungskatalogDialogProps) {
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "leistungsbezeichnung": string | null, // Leistungsbezeichnung\n  "leistungsbeschreibung": string | null, // Beschreibung\n  "einheit": LookupValue | null, // Einheit (select one key: "stunde" | "tag" | "pauschal" | "woche" | "monat") mapping: stunde=Stunde, tag=Tag, pauschal=Pauschal, woche=Woche, monat=Monat\n  "standardpreis": number | null, // Standardpreis (€)\n  "steuersatz_leistung": LookupValue | null, // Steuersatz (select one key: "steuersatz_19" | "steuersatz_7" | "steuersatz_0") mapping: steuersatz_19=19%, steuersatz_7=7%, steuersatz_0=0%\n  "aktiv": boolean | null, // Leistung aktiv\n}`;
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="standardpreis">Standardpreis (€)</Label>
            <Input
              id="standardpreis"
              type="number"
          ...
          <div className="space-y-2">
            <Label htmlFor="steuersatz_leistung">Steuersatz</Label>
            <Select
              value={lookupKey(fields.steuersatz_leistung) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, steuersatz_leistung: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="steuersatz_leistung"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
